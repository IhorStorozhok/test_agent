...
do
  ...
  curl --sS -X POST $HOTHOST_SERVER_BASE/api/data/$HOTHOST_AGENT_SECRET -H 'Content-Type: application/json' -d "$JSON_DATA" -A 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Safari/537.36' 2>&1 | ts | while read line; do echo ${line} ℹ️ HostInfo; done \
